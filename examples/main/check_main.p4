...
control IngressPipeImpl (inout parsed_headers_t    hdr,
    ...
    table udf_rule_table {
        key = {
            standard_metadata.ingress_port: exact; //绑定入向接口，每个接口只能绑定一个acl
	    local_metadata.ip_proto: exact;  //用来表示rule id
        }
        actions = {
            udf_rule_forward;
	    udf_rule_drop;
...
//    action udf_acl_bind(port_num_t port, bit<16> aclId) {
//    }
//    action udf_acl_unbind(port_num_t port, bit<16> aclId) {
//    }
